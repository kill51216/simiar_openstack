...
you want to connect to a similar service via a debugger running on a different
host.

Note that using the remote debug option changes how Similar uses the eventlet
...
Possible Values:
   ...
   * IP address of a remote host as a command line parameter
     to a similar service. For Example:

    /usr/local/bin/similar-api --config-file /etc/similar/similar.conf
...
the port you want to use to connect to a similar service via a debugger running
on different host.
